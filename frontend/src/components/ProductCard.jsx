...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="product-card">
      ...
      <button
        type="button"
        className={`btn btn-block ${isOutOfStock || user?.is_admin ? "btn-outline" : "btn-primary"}`}
        onClick={handleAdd}
        disabled={adding || isOutOfStock || isCartMax || user?.is_admin}
        title={user?.is_admin ? "Admins manage the catalog only and cannot buy products" : undefined}
      >
        {user?.is_admin ? (
