...
export default function ProductDetail() {
  ...
  const handleAdd = async () => {
    ...
    if (user?.is_admin) {
      toast.info("Admin accounts cannot place orders. Only customers can buy products.");
      return;
    }
        ...
        <div className="product-detail-info">
          ...
          {!isOutOfStock && (
            <div className="detail-actions-row">
              ...
              <button
                type="button"
                className={`btn btn-lg ${user?.is_admin ? "btn-outline" : "btn-primary"}`}
                style={{ flex: 1 }}
                onClick={handleAdd}
                disabled={adding || remaining <= 0 || user?.is_admin}
                title={user?.is_admin ? "Admins manage the catalog only and cannot place orders" : undefined}
              >
                {user?.is_admin ? (
                  "Admin Mode (Catalog View Only)"
                ) : remaining <= 0 ? (
                  "All Reserved in Bag"
                ) : adding ? (
                  "Adding..."
